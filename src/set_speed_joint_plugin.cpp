  ...
  class ModelJointControler : public ModelPlugin
  {
    public: void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
    {
      ...
      if (_sdf->HasElement("activate_pid_control"))
          this->activate_pid_control = (_sdf->Get<std::string>("activate_pid_control") == "true");
      ...
      const auto &jointController = this->model->GetJointController();
          ...
          jointController->AddJoint(model->GetJoint("right_joint"));
          jointController->AddJoint(model->GetJoint("left_joint"));

          this->right_name = model->GetJoint("right_joint")->GetScopedName();
          this->left_name = model->GetJoint("left_joint")->GetScopedName();
          
          jointController->SetVelocityPID(this->right_name, common::PID(this->kp, this->ki, this->kd));
          jointController->SetVelocityPID(this->left_name, common::PID(this->kp, this->ki, this->kd));
          
          jointController->SetVelocityTarget(this->right_name, 0.0);
          jointController->SetVelocityTarget(this->left_name, 0.0);
      }
    ...
    if(this->activate_pid_control)
      ...
      const auto &jointController = this->model->GetJointController();
      jointController->SetVelocityTarget(this->right_name, this->right_speed_magn);
          jointController->SetVelocityTarget(this->left_name, this->left_speed_magn);
        }else
        {
            // Apply a small linear velocity to the model.
            ROS_DEBUG("Update Wheel Speed BASIC...");
            this->model->GetJoint("right_joint")->SetVelocity(0, this->right_speed_magn);
            this->model->GetJoint("left_joint")->SetVelocity(0, this->left_speed_magn);
        }
